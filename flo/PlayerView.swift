    ...
    var body: some View {
        if #available(iOS 26, *) {
            GeometryReader {
                        ...
                        VStack(alignment: .leading, spacing: 3) {
                            ...
                            HStack(alignment: .bottom, spacing: 10) {
                                if viewModel.queue.isEmpty {
                                    ...
                                    Text(
                                        "From \(viewModel.nowPlaying.albumName ?? "")"
                                ...
                                ForEach(viewModel.queue.indices, id: \.self) {
                                    ...
                                    HStack(alignment: .top) {
                                        VStack(alignment: .leading) {
                                            ...
                                            Text(
                                                viewModel.queue[idx].artistName
                                                    ?? ""
                                            )
                                            .customFont(.caption1)
                                        }
                ...
                .animation(
                    .spring(duration: 0.4),
                    value: queueDragOffset.height
                )
                .foregroundColor(.primary)
                .zIndex(1)
                .offset(
                    y: showQueue
                        ? UIScreen.main.bounds.height - 500
                            + queueDragOffset.height
                ...
                ZStack {
                    VStack {
                        ...
                        if let image = UIImage(
                            ...
                            LazyImage(
                                url: URL(string: viewModel.getAlbumCoverArt())
                            ) { state in
                        ...
                        HStack(spacing: size.width * 0.15) {
                            ...
                            Button {
                                viewModel.isPlaying
                                    ? viewModel.pause() : viewModel.play()
                            } label: {
                                Image(
                            ...
                            .foregroundColor(
                                viewModel.isMediaLoading ? .gray : .white
                            )
                ...
                .background {
                    ZStack {
                        if UserDefaultsManager.playerBackground
                            == PlayerBackground.translucent
                        {
                            if let image = UIImage(
                                ...
                                LazyImage(
                                    url: URL(
                                        string: viewModel.getAlbumCoverArt()
                                    )
                                ) { state in
                                    if let image = state.image {
                                        image
